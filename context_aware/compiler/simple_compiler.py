...
class SimpleCompiler:
    def compile_search_results(self, items: List[ContextItem]) -> str:
        """
        Compiles search results into a skeleton format (signatures + dependencies).
        """
        output = []
    ...
    def compile_read_result(self, item: ContextItem) -> str:
        """
        Compiles a single item's full content (source code).
        """
        output = []
        output.append("<context_aware_context>")
        output.append(f"  <item id='{item.id}' layer='{item.layer.value}'>")
